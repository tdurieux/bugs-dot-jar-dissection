...
def initParser():
    parser = argparse.ArgumentParser(prog="Bugs.jar", description='Bugs.jar CMD')
    ...
    checkout_parser = subparsers.add_parser('checkout', help='Checkout a bug', parents=[bug_parser])
    checkout_parser.set_defaults(func=checkout_args)
    checkout_parser.add_argument("--destination", "-d", help="The destination")

    info_parser = subparsers.add_parser('info', help='Info of a bug', parents=[bug_parser])
    info_parser.set_defaults(func=info_args)
    info_parser.add_argument("--json", action='store_true', help="Export as JSON")

    compile_parser = subparsers.add_parser('compile', help='Compile of a bug', parents=[bug_parser])
